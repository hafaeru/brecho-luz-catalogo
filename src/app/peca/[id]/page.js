...
// --- NOVO: Componente do Modal da Imagem ---
function ImageModal({ src, alt, onClose }) {
  ...
  return (
    ...
    >
      ...
      <button 
        className="absolute top-4 right-5 text-white text-4xl font-bold hover:text-stone-300 transition-colors"
        onClick={onClose}
      >
        &times;
      </button>
...
export default function PecaDetalhe() {
    ...
    useEffect(() => {
      async function getPeca() {
        ...
        if (!error && data) {
          setPeca(data);
          setImagemAtiva(data.imagens && data.imagens.length > 0 ? data.imagens[0] : '/placeholder.jpg');
        } else {
      ...
      <>
        <div className="container mx-auto p-4 sm:p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
            ...
            <div>
              <div 
                className="relative w-full h-96 md:h-[500px] mb-4 cursor-pointer"
              ...
              <div className="flex space-x-2 overflow-x-auto p-2">
                {peca.imagens?.map((img, index) => (
                  <div key={index} className={`relative w-20 h-20 flex-shrink-0 cursor-pointer border-2 rounded ${imagemAtiva === img ? 'border-yellow-500' : 'border-transparent'}`}
            ...
            <div className="flex flex-col space-y-6">
              ...
              {/* --- CÓDIGO RESTAURADO: CARD DE DETALHES --- */}
              ...
              {/* --- CÓDIGO RESTAURADO: CARD DE MEDIDAS --- */}
              {temMedidas && (
                <div className="bg-stone-50 border border-gray-200 rounded-lg p-4">
